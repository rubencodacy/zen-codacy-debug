...
./autogen.sh
CC="$CC" CXX="$CXX" ./configure --prefix="${PREFIX}" --host="$HOST" --build="$BUILD" "$RUST_ARG" "$HARDENING_ARG" "$LCOV_ARG" "$TEST_ARG" "$MINING_ARG" "$PROTON_ARG" "$LIBS_ARG" "$ADDRESSINDEXING_ARG" --enable-werror CXXFLAGS='-Wno-literal-conversion'
"$MAKE" "$@" V=1
